...
**Format version: 1** (stable as of v1.0.0 — see §3.5)
...
vault is out of contract rather than supported, which is what the pre-1.0
exemption allowed while it was in force (§3.5).
...
preservation for forward compatibility (payload version 1 is the only version
that has shipped, so there are no unknown fields to preserve; until a version 2
exists, §3.5's rule that a reader rejects anything newer covers the case), and
generation metadata on the entry (see §7). The word `id` is reserved for that deferred opaque identifier
...
#### The pre-1.0 exemption, and its end

Before v1.0.0 the format version was marked unstable, and breaking changes were
permitted with no migration path: a vault written by an earlier build could
simply stop being readable, and an invariant could be introduced that existing
files violated. That was the point of shipping a version marked unstable, and
it is why the rules above are written as an obligation that begins at 1.0
rather than one owed from the first commit.

**That exemption ended at v1.0.0.** Every format and payload version that has
been written by a release from then on MUST have a path forward, which in
practice means the migration is written in the same change that bumps the
version. Format version 1 and payload version 1 are the first versions covered,
and currently the only ones.

Vaults written by a pre-1.0 build are not covered. They were produced under the
exemption, and nothing here obliges a 1.x release to read one.

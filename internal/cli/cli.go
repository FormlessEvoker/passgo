...
// parsing and dispatch for each subcommand in docs/SPECIFICATION.md §6, on
...
import (
	...
	"github.com/FormlessEvoker/passgo/internal/vault"
)
...
See docs/SPECIFICATION.md for the full command reference.
`
...
// version is stamped at link time by the release build (see
// .goreleaser.yaml) with the exact tag being released. It is empty in
// every other build, which is the signal to fall back to build info.
//
// The stamp exists because a release binary is cross-compiled from a
// detached checkout in CI, where the VCS state Go reads for Main.Version
// is not guaranteed to be the tag we are publishing. Recording the tag
// explicitly is cheap and removes the guesswork.
var version string

// versionString reports the version of this binary.
//
// A released binary returns the tag stamped into it. Otherwise this
// falls back to the module version Go resolved the binary against —
// e.g. "v0.2.0" for `go install .../passgo@v0.2.0` — read from the
// build info Go embeds automatically, with no ldflags wiring needed. A
// local `go build` inside the repo instead gets Go's git-derived
// pseudo-version (commit hash, "+dirty" if the working tree had
// uncommitted changes), which is more useful for a dev build than a
// flat "dev" string would be.

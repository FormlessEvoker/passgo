...
import (
	...
	"github.com/FormlessEvoker/passgo/internal/crypto"
...
// identity rule in docs/SPECIFICATION.md §3.2: name alone identifies an
...
// docs/SPECIFICATION.md §5 makes: because names are unique, the exact-match
...
// docs/SPECIFICATION.md §3.2, so `add` can no longer produce the duplicate
...
// The release build stamps the tag into `version` at link time. When
// it is set, it must be what `passgo --version` reports — otherwise
// the stamp is decoration and a released binary could name a version
// it is not.
func TestStampedVersionIsReported(t *testing.T) {
	...
	if strings.TrimSpace(out) != "passgo v1.2.3" {
		t.Errorf("--version printed %q, want %q", strings.TrimSpace(out), "passgo v1.2.3")
	}
}

// Without the stamp — every `go build` and `go install` — the version
// still has to come from somewhere. The build-info fallback covers
// that, and the one thing it must never do is report an empty version,
// which would print a bare "passgo" and tell the user nothing.
func TestUnstampedVersionFallsBackToBuildInfo(t *testing.T) {
	orig := version
	version = ""
	t.Cleanup(func() { version = orig })

	if got := versionString(); got == "" {
		t.Error("versionString() was empty with no stamp; want a build-info version or \"dev\"")
	}
}
